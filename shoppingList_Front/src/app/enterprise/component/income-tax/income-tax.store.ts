...
@Injectable()
export class IncomeTaxStore extends ComponentStore<ComponentState> {
  ...
  /* Selector */
  readonly ds$: Observable<Row[]> = this.select(state => state.datasource);
  
  /* Updater */
  readonly initDatasource = this.updater(
    (state) => {

      let ds:Row[] = [ ...state.datasource ];
